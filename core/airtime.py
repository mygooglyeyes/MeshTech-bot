...
length).  The bot stores each message's size, so per-node "time on the
air" is computed from those numbers and labelled **est.** everywhere -
real collisions, retries and LBT backoff are invisible to us.

Defaults match the reference openHop repeater (SF7, 62.5 kHz bandwidth,
coding rate 4/5, 32-symbol preamble); the live values come from the
``radio:`` section of config.yaml so a node on different settings gets
correct numbers by editing config, not code.
...
def airtime_from_settings(payload_bytes: int, radio: Optional[object]) -> float:
    ...
    return lora_airtime_ms(
        ...
        bandwidth_khz=getattr(radio, "bandwidth_khz", 62.5),
...
def radio_summary(radio: Optional[object]) -> Dict[str, object]:
    ...
    return {
        ...
        "bw_khz": getattr(radio, "bandwidth_khz", 62.5),
